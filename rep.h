...
int demo_ad()
{
	...
	T t;
	//train
	std::map<int, std::pair<int, int> > ads;
	std::pair<int, int> all;
	while(1 == fread(&t, sizeof(T), 1, train))
	{
		std::pair<int, int>& p = ads[t.adid];
		p.first += t.click;
		p.second += t.imp;
		all.first += t.click;
		all.second += t.imp;
	...
	{
		double ctr = 0.5;
		std::map<int, std::pair<int, int> >::const_iterator it = ads.find(t.adid);
		if (it != ads.end() && it->second.second > 5)
			ctr = it->second.first * 1.0 / it->second.second;
		else
			ctr = all.first * 1.0 / all.second;
		printf("%d %d %lf\n", t.click, t.imp, ctr);
